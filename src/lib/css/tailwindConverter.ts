...
export class TailwindConverter extends BaseCssConverter implements ICssConverter {
  /**
   * Class-level constant mapping of CSS properties to Tailwind utility class converters
   * Moved to static class property to avoid recreating on every method call
   */
  private static readonly PROPERTY_MAPPINGS: Record<string, (val: string) => string | null> = {
    'flex-direction': (val) => {
   ...
   */
  protected parseStyleString(style: string): Map<string, string> {
   ...
   * Uses class-level constant PROPERTY_MAPPINGS for performance
   */
  private getPropertyClass(property: string, value: string): string | null {
    const mapper = TailwindConverter.PROPERTY_MAPPINGS[property.toLowerCase()];
